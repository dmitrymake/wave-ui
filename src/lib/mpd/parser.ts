...
export const MpdParser = {
  ...
  parseCurrentSong(text: string): Track {
    const data = this.parseKeyValue(text);
    return this._normalizeTrack(data);
  },

  parsePlaylists(text: string): Playlist[] {
    const lines = text.split("\n");
    const items: { playlist: string; "last-modified"?: string }[] = [];
    let current: { playlist: string; "last-modified"?: string } | null = null;
  ...
  parseTracks(rawText: string): Track[] {
    const tracks: Track[] = [];
    let currentTrack: Record<string, string> | null = null;
  ...
  _normalizeTrack(raw: Record<string, string>): Track {
