...
/**
 * IndexedDB wrapper for local track metadata caching.
 * Schema updated to support 'album_artist' indexing.
 */
export const db = {
  open() {
    return new Promise((resolve, reject) => {
      // Ensure DATABASE.VERSION is incremented in constants.js to trigger upgrade
      ...
      request.onupgradeneeded = (e) => {
        ...
        // New index for grouping by Album Artist
  ...
  async bulkAdd(tracks) {
    ...
    return new Promise((resolve, reject) => {
      ...
      tracks.forEach((track) => {
        // Fallback defaults logic is handled in worker, but safety check here
        store.put(track);
      });
  ...
  /**
   * Retrieves unique artists.
   * Logic: Groups by 'album_artist' if available, otherwise 'artist'.
   * This consolidates "Artist feat. X" under "Artist".
   */
  async getArtists() {
    const database = await this.open();
    return new Promise((resolve, reject) => {
      ...
      // Using openCursor to iterate all tracks and normalize artists manually
  ...
  async getAlbums() {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Using nextunique to get unique album names directly from index
  ...
  /**
   * Finds albums for a specific artist.
   * Searches both 'album_artist' and 'artist' indexes to catch features.
   */
  async getArtistAlbums(artistName) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Parallel queries:
      // 1. Where this artist is the Album Artist
      // 2. Where this artist is the Track Artist
      ...
      Promise.all([p1, p2])
        .then(([r1, r2]) => {
          ...
          allTracks.forEach((t) => {
            // Filter logic: Only include if it's the main artist or album artist
            // This prevents loose matches if necessary, but usually we want all associated albums
