...
const sampleTracks: Partial<DbTrack>[] = [
  {
...
describe("db.getFilesMap", () => {
  ...
  it("returns empty map for null input", async () => {
    const result = await db.getFilesMap(null);
    expect(result.size).toBe(0);
  });
...
describe("db.getArtistAlbums", () => {
  ...
  it("returns empty for null input", async () => {
    const albums = await db.getArtistAlbums(null);
    expect(albums).toHaveLength(0);
  });
});
...
describe("db.search", () => {
  it("returns empty for empty query", async () => {
    ...
    expect(await db.search(null)).toHaveLength(0);
  });
